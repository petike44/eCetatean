...
export async function writeAuditEntry({ userId, action, actionType, data = {}, }) {
    ...
    const dataHash = sha256(JSON.stringify({ action, actionType, timestamp, data }));
...
export function verifyHashChain(entries) {
    for (let i = 1; i < entries.length; i++) {
        const expectedRecordHash = sha256(entries[i - 1].record_hash + entries[i].data_hash);
