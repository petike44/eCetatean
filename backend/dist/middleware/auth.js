...
export const requireAuth = createMiddleware(async (c, next) => {
    ...
    if (process.env.AUTH_STUB === 'true') {
        const role = process.env.AUTH_STUB_ROLE || 'citizen';
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return c.json({ success: false, error: 'Token de autentificare lipsă' }, 401);
    }
...
export const requireCivilServant = createMiddleware(async (c, next) => {
    ...
    if (role !== 'civil_servant') {
        return c.json({ success: false, error: 'Acces restricționat funcționarilor publici' }, 403);
    }
    await next();
});
