...
const form: PdfForm = {
  ...
  mapping: [
    field('full_name', 'Nume si prenume', 'profile.full_name', 145, 156, 260, true),
    field('cnp', 'CNP', 'profile.cnp', 145, 184, 210, true),
    field('identity_card', 'CI seria si numarul', 'profile.identity_card', 145, 212, 210, true),
    field('current_address', 'Domiciliu actual', 'profile.full_address', 145, 240, 320, true),
    field('new_address', 'Adresa resedintei solicitate', 'input.new_address', 145, 302, 330, true),
    field('date', 'Data', 'system.today', 145, 680, 120, true),
  ],
  required_inputs: [
    { key: 'new_address', label: 'Adresa resedintei solicitate', required: true },
  ],
}
...
function placementRows(fields: Awaited<ReturnType<typeof analyzePdf>>) {
  return fields.map((item) => {
    ...
    const withinVerticalBox = textBaselineY >= boxBottomY && textBaselineY <= boxBottomY + item.height
...
async function main() {
  ...
  const allValuesFilled = fields.every((item) => !item.required || item.value?.trim())
