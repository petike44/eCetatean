...
pdfRoute.post('/generate', requireAuth, async (c) => {
  ...
  try {
    ...
    if (catalogForm) {
      ...
      const fields = await analyzePdf(catalogForm, sourcePdf, profileData ?? {}, additional_data)
      const pdfBuffer = await fillPdf(sourcePdf, fields, profileData ?? {}, additional_data)
...
async function findCatalogForm(slug: string): Promise<PdfForm | null> {
  ...
  return demoForms().find((form) => form.slug === slug || form.id === slug) ?? null
}
